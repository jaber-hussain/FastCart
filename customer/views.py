...
def order_detail(request, order_id):
    ...
    order = get_object_or_404(store_models.Order, customer=request.user, order_id=order_id)
...
def address_create(request):
    # Get the next URL from query params (default to addresses page)
    next_url = request.GET.get('next', None) or "customer:addresses"
    ...
    if request.method == "POST":
        ...
        return redirect(next_url)  # redirect dynamically

    return render(request, "customer/address_create.html", {"next": next_url})
