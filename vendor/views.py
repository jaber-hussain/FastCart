...
def dashboard(request):
    ...
    # Example: get all unique orders for this vendor
    vendor = request.user
    orders = store_models.Order.objects.filter(
        orderitem__vendor=vendor      # ✅ join through OrderItem
...
@vendor_required
def orders(request):
    orders_list = store_models.Order.objects.filter(vendor=request.user)
...
def order_detail(request, order_id):
    order = get_object_or_404(store_models.Order, vendor=request.user, order_id=order_id)
...
def order_item_detail(request, order_id, item_id):
    order = get_object_or_404(store_models.Order, vendor=request.user, order_id=order_id)
    item = get_object_or_404(store_models.OrderItem, item_id=item_id, order=order, vendor=request.user)
    context = {
        "order": order,
        "item": item,
    }
...
def vendor_categories(request):
    ...
    return render(request, "vendor/categories.html", {"categories": cats})
...
def create_category(request):
    if request.method == "POST":
        ...
        image = request.FILES.get("image")
        slug = slugify(title)
        store_models.Category.objects.create(title=title, image=image, slug=slug)
        messages.success(request, "Category created")
        return redirect("vendor:vendor_categories")
    return render(request, "vendor/category_form.html")

@vendor_required
def update_category(request, id):
    cat = get_object_or_404(store_models.Category, id=id)
    if request.method == "POST":
        ...
        return redirect("vendor:vendor_categories")
    return render(request, "vendor/category_form.html", {"category": cat})
...
def create_category_ajax(request):
    """
    Create a category directly from the create product page via AJAX.
    """
    if request.method == "POST":
        title = request.POST.get("title", "").strip()
        if not title:
            return JsonResponse({"success": False, "error": "Category title is required."}, status=400)

        # Create category
        slug = slugify(title)
        category = store_models.Category.objects.create(title=title, slug=slug)
...
def add_category(request):
    if request.method == 'POST':
        title = request.POST.get('title')
        image = request.FILES.get('image')
        store_models.Category.objects.create(title=title, image=image)
        return redirect('manager_dashboard')  # Change to your dashboard name
